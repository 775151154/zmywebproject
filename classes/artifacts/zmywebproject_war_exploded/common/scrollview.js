...
var scrollview = $.extend({}, $.fn.datagrid.defaults.view, {
	...
	render: function(target, container, frozen){
		...
		for(var i=0; i<tableRows.length; i++){
			if(ckrow[tableRows[i]]==1){
				$(target).datagrid('checkRow',tableRows[i]);
			}
		}
		if(isNotNull($("input[type='checkbox']")[1])){
			$("input[type='checkbox']")[1].checked = ckrow["all"];
		}
	},
		...
		for(var i=0; i<fields.length; i++){
			...
			if (col){
				...
				if (col.checkbox){
					...
					tableRows[tableRowIndex++]=rowIndex;
				} else if (col.expander) {
	...
	getRowIndex: function(jq, id){
		...
		if (opts.view.type == 'scrollview'){
			...
			if (index == -1){
				...
				return index + opts.view.index-10;
			}
		} else {
...
function isAllChecked(){
	for(var i=0;i<currRows.length;i++){
		if(ckrow[i]==0){
			...
			$("input[type='checkbox']")[1].checked = ckrow["all"];
			return;
		}else if(ckrow[i]==1&&i==(currRows.length-1)){
			ckrow["all"] = true;
			$("input[type='checkbox']")[1].checked = ckrow["all"];
			return;
		}
